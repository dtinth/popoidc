...
decide for themselves whether that key is authorized — sshid never asserts a
human identity.
...
```bash
ISS=https://sshid.example
KEY="$(cat ~/.ssh/id_ed25519.pub)"
C=$(curl -sG "$ISS/challenge" --data-urlencode "key=$KEY" --data-urlencode "aud=octo-sts.dev")
SIG=$(printf %s "$C" | ssh-keygen -Y sign -n sshid -f ~/.ssh/id_ed25519 2>/dev/null)
...
- `SSHID_ISSUER` — public HTTPS issuer URL (equals `iss`).
- `SSHID_SIGNING_JWK` — RSA private JWK with a `kid` (`deno task keygen`).
- `SSHID_HMAC_SECRET` — secret for the challenge HMAC.
- `SSHID_NAMESPACE` — optional SSHSIG namespace (default `sshid`).
- `PORT` — optional (default `8000`).

sshid is **stateless**: no database. The signing key and HMAC secret live in
env.
...
```bash
docker build -t sshid .
docker run -p 8000:8000 --env-file .env sshid
```
